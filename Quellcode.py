...
for i, history in enumerate(histories):
    plot_loss(history, i + 1)

def plot_predictions(predicted, actual, company):
    plt.figure(figsize=(10, 6))
    plt.plot(actual, label='Actual Prices')
    plt.plot(predicted, label='Predicted Prices')
    plt.title(f'Predicted vs Actual Prices for {company}')
    plt.xlabel('Time')
    plt.ylabel('Price')
    plt.legend()
    plt.show()

actual_prices = {company: data[price_col].values[:199] for company, price_col in zip(companies, price_cols)}

for company in companies:
    plot_predictions(predicted_prices_df[company], actual_prices[company], company)

def plot_error_distribution(predicted, actual, company):
    errors = predicted - actual
    plt.figure(figsize=(10, 6))
    plt.hist(errors, bins=30, edgecolor='k')
    plt.title(f'Error Distribution for {company}')
